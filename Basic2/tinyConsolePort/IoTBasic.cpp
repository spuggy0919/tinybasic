...
bool checkargc0is(); /* check argv[0] is tb for shell mode, ti for interactive mode spuggy0919*/
...
#ifdef HASARGS
	if (bargc > 1 && ifileopen(bargv[1])) { /* bug cmd argc=2 with argv[1]*/
		xload(bargv[1]);
		st=SRUN;
		ifileclose();
		// bnointafterrun=TERMINATEAFTERRUN;
		// return 1;	
		bnointafterrun=checkargc0is() ? 1:0; /*ti interactive, tb shell cmd spuggy0919*/
		return 	checkargc0is();		
	}
#endif
...
/* check for the break character */
	...
	if (buffer[i] == CTRL_CKEY) return -1; /*control-c spuggy0919*/
...
void xcolor() {
	...
	case 3:
		...
		rgbcolor(r, g, b);
		break;
	case 4: /*spuggy0919 support rgba to meet javascript canvas*/
		a=pop();
		b=pop();
		g=pop();
		r=pop();
 ...
 */
		...
		if (checkch() == BREAKCHAR||checkch() == CTRL_CKEY) {
			st=SINT; 
 ...
 */
extern int forceexit;/*spuggy0919 for shell mode*/

bool tbloop_200() { /* spuggy change return bool*/
...
// #ifdef ARDUINO  /*spuggy0919 tinyconsole use main*/
int tbmain_200(int argc, char* argv[]){  /*spuggy0919*/
...
extern int tbmain_200(int argc, char* argv[]);
bool checkargc0is(){
